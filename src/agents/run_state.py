...
class RunState(Generic[TContext, TAgent]):
    ...
    def _serialize_input_guardrail_results(self) -> list[dict[str, Any]]:
        """Serialize input guardrail results."""
        return [
            {
    ...
    def _serialize_output_guardrail_results(self) -> list[dict[str, Any]]:
        """Serialize output guardrail results."""
        return [
            {
    ...
    def _camelize_field_names(data: dict[str, Any] | list[Any] | Any) -> Any:
        ...
        if isinstance(data, dict):
            ...
            for key, value in data.items():
                camelized_key = _SNAKE_TO_CAMEL_FIELD_MAP.get(key, key)
    ...
    def to_json(self) -> dict[str, Any]:
        ...
        result = {
            ...
            "inputGuardrailResults": self._serialize_input_guardrail_results(),
            "outputGuardrailResults": self._serialize_output_guardrail_results(),
        }
    ...
    def _serialize_processed_response(
        ...
        handoffs = self._serialize_handoffs(processed_response.handoffs)

        functions = self._serialize_tool_actions(
            processed_response.functions,
            tool_attr="function_tool",
            wrapper_key="tool",
            include_description=True,
            include_params_schema=True,
        )

        computer_actions = self._serialize_tool_actions(
            processed_response.computer_actions,
            tool_attr="computer_tool",
            wrapper_key="computer",
            include_description=True,
        )

        local_shell_actions = self._serialize_tool_actions(
            processed_response.local_shell_calls,
            tool_attr="local_shell_tool",
            wrapper_key="localShell",
            include_description=True,
        )

        shell_actions = self._serialize_tool_actions(
            processed_response.shell_calls,
            tool_attr="shell_tool",
            wrapper_key="shell",
            include_description=True,
        )

        apply_patch_actions = self._serialize_tool_actions(
            processed_response.apply_patch_calls,
            tool_attr="apply_patch_tool",
            wrapper_key="applyPatch",
            include_description=True,
        )

        mcp_approval_requests = self._serialize_mcp_approval_requests(
        ...
        return {
            ...
            "localShellActions": local_shell_actions,
...
async def _deserialize_processed_response(
    ...
    handoffs = _deserialize_actions(
        processed_response_data.get("handoffs", []),
        tool_key="handoff",
        tool_map=handoffs_map,
        call_parser=lambda data: ResponseFunctionToolCall(**data),
        action_factory=lambda tool_call, handoff: ToolRunHandoff(
            tool_call=tool_call, handoff=handoff
        ),
        name_resolver=lambda data: data.get("handoff", {}).get("toolName")
        or data.get("handoff", {}).get("tool_name"),
    )

    functions = _deserialize_actions(
        processed_response_data.get("functions", []),
        tool_key="tool",
        tool_map=tools_map,
        call_parser=lambda data: ResponseFunctionToolCall(**data),
        action_factory=lambda tool_call, function_tool: ToolRunFunction(
            tool_call=tool_call, function_tool=function_tool
        ),
    )

    computer_actions = _deserialize_actions(
        processed_response_data.get("computerActions", []),
        tool_key="computer",
        tool_map=computer_tools_map,
        call_parser=lambda data: ResponseComputerToolCall(**data),
        action_factory=lambda tool_call, computer_tool: ToolRunComputerAction(
            tool_call=tool_call, computer_tool=computer_tool
        ),
    )

    local_shell_actions = _deserialize_actions(
        processed_response_data.get("localShellActions", []),
        tool_key="localShell",
        tool_map=local_shell_tools_map,
        call_parser=lambda data: _parse_with_adapter(_LOCAL_SHELL_CALL_ADAPTER, data),
        action_factory=lambda tool_call, local_shell_tool: ToolRunLocalShellCall(
            tool_call=tool_call, local_shell_tool=local_shell_tool
        ),
    )

    shell_actions = _deserialize_actions(
        processed_response_data.get("shellActions", []),
        tool_key="shell",
        tool_map=shell_tools_map,
        call_parser=lambda data: _parse_with_adapter(_LOCAL_SHELL_CALL_ADAPTER, data),
        action_factory=lambda tool_call, shell_tool: ToolRunShellCall(
            tool_call=tool_call, shell_tool=shell_tool
        ),
    )

    apply_patch_actions = _deserialize_actions(
        processed_response_data.get("applyPatchActions", []),
        tool_key="applyPatch",
        tool_map=apply_patch_tools_map,
        call_parser=_parse_apply_patch_call,
        action_factory=lambda tool_call, apply_patch_tool: ToolRunApplyPatchCall(
...
def _normalize_field_names(data: dict[str, Any]) -> dict[str, Any]:
    ...
    for key, value in data.items():
        normalized_key = _CAMEL_TO_SNAKE_FIELD_MAP.get(key, key)
...
def _deserialize_input_guardrail_results(
    ...
    for entry in results_data or []:
        guardrail_info = entry.get("guardrail", {}) if isinstance(entry, dict) else {}
        name = guardrail_info.get("name") or "deserialized_input_guardrail"
        output_data = entry.get("output", {}) if isinstance(entry, dict) else {}
        tripwire = bool(output_data.get("tripwireTriggered"))
        ...
        def _guardrail_fn(
            ...
            input: Any,
            *,
            _info: Any = output_info,
            _tripwire: bool = tripwire,
        ) -> GuardrailFunctionOutput:
            return GuardrailFunctionOutput(output_info=_info, tripwire_triggered=_tripwire)

        guardrail = InputGuardrail(guardrail_function=_guardrail_fn, name=name)
        deserialized.append(
            InputGuardrailResult(
                guardrail=guardrail,
...
def _deserialize_output_guardrail_results(
    ...
    *,
    ...
    for entry in results_data or []:
        guardrail_info = entry.get("guardrail", {}) if isinstance(entry, dict) else {}
        name = guardrail_info.get("name") or "deserialized_output_guardrail"
        output_data = entry.get("output", {}) if isinstance(entry, dict) else {}
        tripwire = bool(output_data.get("tripwireTriggered"))
        output_info = output_data.get("outputInfo")
        agent_output = entry.get("agentOutput") if isinstance(entry, dict) else None
        agent_data = entry.get("agent") if isinstance(entry, dict) else None
        ...
        def _guardrail_fn(
            ...
            *,
            ...
            return GuardrailFunctionOutput(
                output_info=_info,
                tripwire_triggered=_tripwire,
            )

        guardrail = OutputGuardrail(guardrail_function=_guardrail_fn, name=name)
        agent_name_str = agent_name if isinstance(agent_name, str) else None
        if agent_name_str and agent_name_str in agent_map:
            agent_resolved: Agent[Any] = agent_map[agent_name_str]
        else:
            agent_resolved = fallback_agent
        deserialized.append(
            OutputGuardrailResult(
                guardrail=guardrail,
...
def _convert_protocol_result_to_api(raw_item: dict[str, Any]) -> dict[str, Any]:
    """Backward-compatible alias that normalizes protocol function results to API format."""
    return cast(dict[str, Any], ensure_function_call_output_format(raw_item))
