...
class RunImpl:
    ...
    async def execute_tools_and_side_effects(
        ...
        *,
        ...
        approval_items_by_call_id: dict[str, ToolApprovalItem] = {}
        for item in pre_step_items:
            if not isinstance(item, ToolApprovalItem):
                continue
            approval_call_id = _extract_tool_call_id(item.raw_item)
            if approval_call_id:
                approval_items_by_call_id[approval_call_id] = item
        ...
        if mcp_requests_requiring_manual_approval:
            seen_mcp_request_ids: set[str] = set()
            for mcp_request in mcp_requests_requiring_manual_approval:
                request_item = mcp_request.request_item
                request_id = _extract_mcp_request_id_from_run(mcp_request)
                if request_id and request_id in seen_mcp_request_ids:
                    continue
                if request_id:
                    seen_mcp_request_ids.add(request_id)

                tool_name = RunContextWrapper._to_str_or_none(getattr(request_item, "name", None))
                tool_name = tool_name or mcp_request.mcp_tool.name

                existing_pending = approval_items_by_call_id.get(request_id or "")
                approval_status = context_wrapper.is_tool_approved(tool_name, request_id or "")
                if approval_status is None and existing_pending:
                    fallback_tool_name = RunContextWrapper._resolve_tool_name(existing_pending)
                    approval_status = context_wrapper.is_tool_approved(
                        fallback_tool_name, request_id or ""
                    )
                if approval_status is not None:
                    if approval_status is True and request_id:
                        ...
                        new_step_items.append(
                            MCPApprovalResponseItem(raw_item=approval_response_raw, agent=agent)
                        )
                    continue
                ...
                new_step_items.append(approval_item)
        ...
        *,
        ...
        for approval in pending_approval_items:
            if not isinstance(approval, ToolApprovalItem):
                continue
            approval_call_id = _extract_tool_call_id(approval.raw_item)
            if approval_call_id:
                approval_items_by_call_id[approval_call_id] = approval
        ...
        # Surface hosted MCP approvals without callbacks so callers can approve manually.
        for mcp_request in mcp_requests_requiring_manual_approval:
            request_item = _get_mapping_or_attr(mcp_request, "request_item")
            request_id = _extract_mcp_request_id_from_run(mcp_request)
            existing_pending = approval_items_by_call_id.get(request_id) if request_id else None

            tool_name = None
            if isinstance(request_item, Mapping):
                tool_name = request_item.get("name")
            elif hasattr(request_item, "name"):
                tool_name = request_item.name
            tool_name = RunContextWrapper._to_str_or_none(tool_name) or mcp_request.mcp_tool.name
        ...
        for run in processed_response.functions:
            call_id = run.tool_call.call_id
            approval_status = context_wrapper.is_tool_approved(run.function_tool.name, call_id)
        ...
        *,
        ...
        if needs_approval_result:
            ...
            if approval_status is False:
                ...
                return _shell_rejection_item(agent, shell_call.call_id)
...
def _process_hosted_mcp_approvals(
    *,
    ...
    for mcp_run in mcp_approval_requests:
        ...
        approved = context_wrapper.is_tool_approved(tool_name=tool_name, call_id=request_id)
